...
DEVICE_ID=$(xcrun simctl list devices available | grep -E "iPhone (17|16|15)" | head -1 | grep -oE "[0-9A-F]{8}-[0-9A-F]{4}-[0-9A-F]{4}-[0-9A-F]{4}-[0-9A-F]{12}" | head -1)
...
while true; do
  ...
  FILENAME="$SCREENSHOT_DIR/iPhone_69-$(printf '%02d' $COUNTER).png"
